<x-auth-layout :title="__('2FA Challenge')">
    <div class="w-full max-w-md overflow-hidden text-gray-500 bg-white shadow rounded-xl">
        <div class="w-full md:flex">
            ...
            <div x-data="{ useRecoveryCode: false }" class="w-full px-5 py-10 md:px-10">
                <form action="{{ route('two-factor.login') }}" method="POST" autocomplete="off"> @csrf
                    <div class="text-left">
                        <h1 class="text-2xl font-bold text-gray-900">Two Factor Authentication</h1>
                        <p x-show="!useRecoveryCode" class="mt-4 text-sm">
                    ...
                    <x-auth-validation-errors class="px-3 py-2 mt-6 text-xs" :errors="$errors" />

                    <div class="flex flex-col mt-8 space-y-4">
                        <div class="w-full">
                            <label for="useRecoveryCode === true ? 'recovery_code' : 'code'" class="sr-only">
                                OTP token:
                            </label>
                            <div class="flex">
                                <div class="z-10 flex items-center justify-center w-10 pl-1 text-center pointer-events-none">
                                    <x-heroicon-o-shield-check class="w-auto h-4 text-gray-400"/>
                                </div>
                                <input
                                    type="text"
                ...
                <div class="mt-8 text-center">
                    <p x-show="!useRecoveryCode" class="mt-4 text-sm">
                        Lost your 2FA devices?
                        <button type="button" @click="useRecoveryCode = !useRecoveryCode" class="text-primary-700 hover:text-primary-500 focus:outline-none">
                            Use recovery code
                        </button>
                    </p>
                    <p x-show="useRecoveryCode" class="mt-4 text-sm">
                        Already receive or have code?
                        <button type="button" @click="useRecoveryCode = !useRecoveryCode" class="text-primary-700 hover:text-primary-500 focus:outline-none">
                            Use OTP Token
                        </button>
                    </p>
