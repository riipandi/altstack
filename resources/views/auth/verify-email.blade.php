<x-auth-layout :title="__('Verify Email Address')">
            ...
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                <div class="text-left">
                    ...
                    <p class="text-sm mt-4">
                        {{ __('Before starting, you must verify your email address by clicking on the link we just sent.') }}
                    </p>
                    <p class="text-sm mt-4">
                        Didn't receive an email with a link for verification?
                        We will be happy to send you another email.
                    </p>
                </div>

                @if (session('status') == 'verification-link-sent')
                <div class="mt-6 py-3 px-2 text-xs bg-green-50 border-l-4 border-green-400 rounded-sm">
                    <div class="flex">
                ...
                <div class="flex flex-col space-y-4 mt-6">
                    <form action="{{ route('verification.send') }}" method="POST" class="space-y-4" autocomplete="off">
                        @csrf
                        <div>
                            <button type="submit" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white border border-transparent rounded-md shadow-sm bg-primary-600 hover:bg-primary-700 focus:outline-none disabled:opacity-25 transition ease-in-out duration-150">
                                <x-heroicon-s-check-circle class="w-4 h-4 mr-2" />
                                {{ __('Resend the verification link') }}
                            </button>
                        </div>
                    </form>
                </div>
                <div class="text-center mt-12 text-sm">
                    <form method="POST" action="{{ route('logout') }}"> @csrf
                        <span class="text-gray-600">You can also cancel this session.</span>
