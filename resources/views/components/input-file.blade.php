<div x-data="{ {{ $name }}: null }" id="{{ $id }}-container" class="relative block w-full bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none fileupload-container">
    <input type="file" x-ref="file" @change="{{ $name }} = $refs.file.files[0].name" id="{{ $id }}" name="{{ $name }}" class="absolute inset-0 z-50 w-full h-full p-0 m-0 outline-none opacity-0"
        ...
        <div class="flex justify-center px-6 pt-5 pb-6 rounded-md">
            <div class="space-y-1 text-center">
                ...
                <div class="flex text-sm tracking-tight text-gray-600">
                    <a href="javascript:void(0)" class="relative font-medium bg-white rounded-md cursor-pointer text-primary-600 hover:text-primary-500 focus-within:outline-none">
                        Upload a file
                    </a>
